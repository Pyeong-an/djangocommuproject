...
from sub_log_game.models import SubLogList_game
...
#가챠 시스템 구현
def dice(request):
    return render(request,'gatya2.html')
def gatya(request):
    ...
    if mang<=1:
        ...
        return render(request,'gatyaVSSR2.html',{'data':data})
...
def Password_not_in_CharList(password):
    ...
    if not CharList.objects.filter(password=password):
        ...
        return False
def Mainlog_not_in_SubLogList_game(MainLog_id):
    """
    Mainlog가 SubLogList_game에 존재하는지 확인
    없을 경우 True가 반환된다
    """
    if not SubLogList_game.objects.filter(mainlog=MainLog_id):
        return True
    else:
        return False
def Sublog_not_in_SubLogList_game(SubLog_id):
    """
    Mainlog가 SubLogList_game에 존재하는지 확인
    없을 경우 True가 반환된다
    """
    if not SubLogList_game.objects.filter(id=SubLog_id):
        return True
    else:
        return False
...
def Warning_site(request):
    return render(request, 'wrong_char2.html')
...
def contents_replace(request):
    ...
    contents = contents.replace('<<', '<br><br>&nbsp;<a href="/game/log/')
...
def move_detailpage(lg):
    ...
    """
    return redirect('/game/log/'+str(lg) +'#rep',)
#최접 불러오는 거
def latest_char():
    """
    최근 접속자 목록을 넘겨주는 함수
    ...
    for latest_sublog in subloglist:
        if latest_sublog.charname not in latest_charname:
            ...
            log_time = latest_sublog.write_dttm
            write_time = (today - log_time).seconds
            if write_time <= 300 and write_time>=0 : #차가 5분 이하일 경우
                latest.append(latest_sublog.charname+" "+str(write_time)+"초")
                latest_charname.append(latest_sublog.charname)

    subloglist_game = SubLogList_game.objects.all().order_by('-id')[:10]#보드의 정렬
    for latest_sublog in subloglist_game:
        if latest_sublog.charname not in latest_charname:
...
def main_POST(request):
    ...
    if Password_not_in_CharList(password):
        ...
        jungbok_test_log = LogList_game.objects.filter(password=password)#조금이라도 트래픽을 줄이기 위해 함수로 합치지 않았다!
        for log in jungbok_test_log:
            if contents == log.contents and datetime.today().minute == log.write_dttm.minute:
                return Warning_site(request)

        log = LogList_game() #빈 로그 오브젝트 생성
        sublog = SubLogList_game() #빈 서브로그 오브젝트 생성
...
def detail_POST(request,lg):
    ...
    if Password_not_in_CharList(password):
        ...
        sublog = SubLogList_game() #빈 서브로그 생성
        ...
        jungbok_test_log = SubLogList_game.objects.filter(mainlog=lg)#트래픽을 줄이기위해 일부러 함수로 빼지 않았다!
        for log in jungbok_test_log:
            if contents == log.contents and datetime.today().minute == log.write_dttm.minute:
                return Warning_site(request)
...
def nextpage_False(loglist_game,t):  
    """
    다음 페이지가 있는지 확인하는 함수
    """
    try:
...
def loglist_game_old(loglist_game):
    """
    세션에 저장된 것이 펴기 모드일 때 데이터 불러오는 함수
    """
    subloglist_game_list = [] #서브로그리스트의 리스트를 딕셔너리로 넘겨주기 위해 배열 선언
    for i in loglist_game[0:10]:#로그리스트에 들어 있는 '메인로그'들을 하나씩 가져온다
        subloglist_game = SubLogList_game.objects.filter(mainlog=i.id).order_by('id')#메인로그의 id값으로 서브로그 목록을 찾아내어 리스트로 끌고 온다
        subloglist_game_list.append(subloglist_game)#subloglist_list에 append
    return subloglist_game_list
...
def main_GET_data(request,page):
    ...
    loglist_game = LogList_game.objects.all().order_by('-id')[(page-1)*10:page*10+1]#보드의 정렬
    nextpage = nextpage_False(loglist_game,10)
    ...
    if main_type == 'old':#메인타입이 올드일 경우
        subloglist_game_list = loglist_game_old(loglist_game)
        
        return {'logs':subloglist_game_list, 'latest':latest,'page':[page-1,page,page+1], 'next':nextpage,'main_type':main_type, 'choijeob':choijeob, 'where':where,'password':password,'notice':notice}

    return {'logs':loglist_game[0:10], 'latest':latest,'page':[page-1,page,page+1], 'next':nextpage,'main_tpye':main_type, 'choijeob':choijeob, 'where':where,'password':password,'notice':notice}
def detail_GET_data(request,lg):
    """
    디테일 데이터 리턴하는 함수
    """
    subloglist_game = SubLogList_game.objects.filter(mainlog=lg).order_by('id')
    password = request.session.get('password',"")
    where = request.session.get('where',"")
    return {'logs': subloglist_game,'password':password,'where':where}
...
def sublog_count_plus(lg):
    ...
    """
    mainlg = LogList_game.objects.get(id=lg)
...
def main_page(request,page=1):
    ...
    if request.method == 'POST':#메인 페이지에서 포스트 요청을 받았을 때
        ...
        return render(request, 'main_page2.html',{'data' : data})
def choose_paging(request):
    ...
    return redirect('/game/main/'+page)
#상세 페이지
def log_detail(request, lg):
    """
    /game/log/<int:lg> 일때 실행되는 함수
    """
    if Mainlog_not_in_SubLogList_game(lg):
        return Warning_site(request)
    ...
    if request.method == 'POST':
        ...
        return render(request, 'log_detail2.html',{'data': data})
#수정 페이지
def log_confirm(request,lg):
    """
    로그를 수정하려고 할 때, 비밀번호 확인하는 함수
    """
    if Sublog_not_in_SubLogList_game(lg):#잘못된 로그 주소로 접속했을 때 오류를 띄운다
        return Warning_site(request)

    if request.method == "POST":
        
        sublog = SubLogList_game.objects.get(id=lg)
        ...
        if modify_password_not_match(request,lg,sublog):#비밀번호가 틀릴때    
            ...
            return render(request, 'log_modify2.html', {'data':data})
    else:

        sublog = SubLogList_game.objects.get(id=lg)
        return render (request, 'password_confirm2.html',{'log':sublog})
def log_modify(request,lg):
    ...
    if request.method == "POST":
        sublog = SubLogList_game.objects.get(id=lg)#고칠 서브로그를 불러와서 수정!!
        password = request.POST.get('password',None)
        where = request.POST.get('where',None)
        if Password_not_in_CharList(password):
        ...
        if sublog == SubLogList_game.objects.filter(mainlog=sublog.mainlog).order_by('id')[0]:
            #서브로그리스트를 차례대로 정렬한 것의 제일 첫번째와 서브로그가 일치하는 경우 메인로그도 고쳐야 한다
            
            log = LogList_game.objects.get(id=sublog.mainlog)
...
def log_delete(request,lg):
    if not SubLogList_game.objects.filter(id=lg):
        return render(request, 'wrong_char2.html')
    sublog = SubLogList_game.objects.get(id=lg)
    if request.method == "POST":
        password = request.POST.get('password',None)
        if password != sublog.password:#비밀번호가 틀릴때
            ...
            if sublog == SubLogList_game.objects.filter(mainlog=sublog.mainlog).order_by('id')[0]:
                ...
                subloglist_game = SubLogList_game.objects.filter(mainlog=sublog.mainlog).order_by('id')
                for subl in subloglist_game:
                    char = CharList.objects.get(password=subl.password)
                    log_minus(char)
                    subl.delete()

            else:
                mainlg = LogList_game.objects.get(id=sublog.mainlog)
                mainlg.sublog_count -= 1
                mainlg.save()
        ...
        return render (request, 'delete_confirm2.html',{'log':sublog})

def char_ranking(request):

    ranking = charanking.objects.all().order_by('-logcount')
    return render(request, 'char_ranking2.html',{"rank":ranking})
def search_choose_paging(request):
    ...
    return redirect('/game/search/'+page+'?search_word='+search_word)

def log_search(request,page=1):
    search_word = request.GET.get('search_word',None)
    search_list = list(set(search_word.split(" ")))#스플릿 후 중복 제거
    all_subloglist_game = SubLogList_game.objects.all().order_by('-id')#모든 sublog를 역순 정렬
    ori_search_result = []
    log_id = ""
    first_charname = search_list[0]
    for sublog in all_subloglist_game:
    ...
    del all_subloglist_game
    search_result = []
    
    for charname in search_list[1:]:
        for mainlg in ori_search_result:
            search_in_mainlog = SubLogList_game.objects.filter(mainlog=mainlg)#메인로그에 있는 sublog를 다불러온다
    ...
    for mainlg in ori_search_result[first_log:last_log]:
        log = LogList_game.objects.get(id=mainlg)
        loglist_game.append(log)
    ...
    if main_type == 'old':#메인타입이 올드일 경우
        subloglist_game_list = loglist_game_old(loglist_game)
        data = {'logs':subloglist_game_list, 'latest':latest,'page':[page-1,page,page+1], 'search_word':search_word, 'next':nextpage, 'main_type':main_type, 'choijeob':choijeob, 'where':where,'password':password ,'notice':notice}

    else:
        data = {'logs':loglist_game, 'latest':latest,'page':[page-1,page,page+1], 'search_word':search_word, 'next':nextpage, 'main_type':main_type, 'choijeob':choijeob, 'where':where,'password':password ,'notice':notice}


    return render(request, 'search_page2.html',{'data' : data})
#구버전 세션 저장용
...
def save_log(request,page=1):
    ...
    if main_type != 'old':
        ...
        data = main_GET_data(request,page)
        return render(request, 'save_log2.html',{'data' : data})
...
def save_detail_log(request, lg):
    ...
    if Mainlog_not_in_SubLogList_game(lg):
        ...
        data = detail_GET_data(request,lg)
        return render(request, 'save_detail_log2.html',{'data': data})
...
def world(request,page=1):
    ...
    weight 페이지로 이동하는 함수
    """
    world_page = 'weight' + str(page) +'2.html'
